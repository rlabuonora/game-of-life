...
class App extends Component {
  constructor(props) {
    super(props);
    var initialGrid = this.emptyGrid();
    this.state = {
      t: 0,
      on: false,
      data: initialGrid,
  ...
  indexInBounds(i, j) {
    return (
      i >= 0 && i < this.props.rows - 1 && (j >= 0 && j < this.props.cols - 1)
    );
  }

  step(oldGrid) {
    // solves for the next grid (brute force)
  ...
  toggleCell(nested_arr, i, j) {
    return nested_arr.map(function(arr, idx) {
      if (idx !== i) return arr;
        ...
        return arr.map(function(cell, idx) {
  ...
  start() {
    var intervalId = setInterval(this.tick.bind(this), this.state.speed);
    this.setState({
      on: true,
  ...
  slowDown() {
    ...
    if (this.state.speed >= 5000) return;
      ...
      <div className="App">
        <div className="container">
          ...
          <div className="row">
            ...
            <SlowDownButton slowDown={this.slowDown.bind(this)} />
            <SpeedUpButton speedUp={this.speedUp.bind(this)} />
          </div>
        </div>
      </div>
    );
...
class Generation extends Component {
  render() {
    return <h3>Generation: {this.props.t}</h3>;
  }
}
...
class SlowDownButton extends Component {
  render() {
    return (
      <button onClick={this.props.slowDown} className="btn btn-primary">
...
class SpeedUpButton extends Component {
  render() {
    return (
      <button onClick={this.props.speedUp} className="btn btn-primary">
